...
console.log(chalk.whiteBright('            🌙 Bienvenido al núcleo de la Bot Mash 🌙'))
console.log(chalk.whiteBright('     Prepara tu sesión... Mash no puede esperar para servirte querido usuario ✨'))
console.log(chalk.magentaBright('⌬──══─┈•⤣⚡⤤•┈─══──⌬\n'))
...
do {
phoneNumber = await question(chalk.bgBlack(chalk.bold.greenBright(`✦ Por favor, Ingrese el número de WhatsApp.\n${chalk.bold.yellowBright(`✏  Ejemplo: 201021×××××`)}\n${chalk.bold.magentaBright('---> ')}`)))
...
if (connection == 'open') {
console.log(chalk.bold.green('\n❀ Mash-Bot Conectada con éxito ❀'))
}
let reason = new Boom(lastDisconnect?.error)?.output?.statusCode
if (connection === 'close') {
if (reason === DisconnectReason.badMashsession) {
console.log(chalk.bold.cyanBright(`\n⚠︎ SIN CONEXIÓN, BORRE LA CARPETA ${global.Mashsession} Y ESCANEA EL CÓDIGO QR ⚠︎`))
...
let prekey = []
let directorio = readdirSync(`./Mashsession`)
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-')
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./Mashsession/${files}`)
})
} 
...
const directories = [`./Mashsession/`, `./${jadi}/`]
...
setInterval(async () => {
if (stopped === 'close' || !conn || !conn.user) return
await purgeMashsession()
console.log(chalk.bold.cyanBright(`\n╭» ❍ ${global.Mashsession} ❍\n│→ SESIONES NO ESENCIALES ELIMINADAS\n╰― ― ― ― ― ― ― ― ― ― ― ― ― ― ― ― ― ― ― ⌫ ♻`))}, 1000 * 60 * 10) // 10 min

setInterval(async () => {
if (stopped === 'close' || !conn || !conn.user) return
await purgeMashsessionB()}, 1000 * 60 * 10)  
